...
extension UIViewController
{
    ...
    func isValidEmailAdd(_ email: String) -> Bool {
        let emailRegEx =
        "(?:[a-zA-Z0-9!#$%\\&'*+/=?^_`{|}~-]+(?:\\.[a-zA-Z0-9!#$%\\&'*+/=?^_`{|}~-]+)*|\"(?:[\u{001}-\u{008}\u{00B}-\u{00C}\u{00E}-\u{01F}\u{021}-\u{05B}\u{05D}-\u{07F}]|\\\\[\u{0001}-\u{09F}])*\")@(?:(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,}|\\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\\.|$)){4}\\])"
    ...
    func showPopup(withTitle: String = "", message: String, okClicked: @escaping () -> ()){
        ...
        if let popoverController = alert.popoverPresentationController {
            ...
            // Or if you have a bar button item
            // popoverController.barButtonItem = yourBarButtonItem
        }
...
//    }
    ...
    static func getSizeForBoldText(text: String, font: UIFont, maxSize: CGSize) -> CGSize {
        let attrString = getBoldText(text: text, font: font)
        let rect = attrString.boundingRect(with: maxSize, options: NSStringDrawingOptions.usesLineFragmentOrigin, context: nil)
        return CGSize(width: rect.size.width, height: rect.size.height)
    }
