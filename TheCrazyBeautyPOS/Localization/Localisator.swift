...
class Localisator {
    ...
    private var availableLanguagesArray         = ["English_en", "China_zh", "Vietnamese_vi"]
    ...
    var saveInUserDefaults:Bool {
        get {
            return (userDefaults.object(forKey: global().kSaveLanguageDefaultKey) != nil)
        }
        set {
            if newValue {
                userDefaults.set(currentLanguage, forKey: global().kSaveLanguageDefaultKey)
            } else {
                userDefaults.removeObject(forKey: global().kSaveLanguageDefaultKey)
            }
    ...
    // MARK: - Init method
    init() {
        if let languageSaved = userDefaults.object(forKey: global().kSaveLanguageDefaultKey) as? String {
    ...
    fileprivate func localizedStringForKey(_ key:String) -> String {
        if let languageSaved = userDefaults.object(forKey: global().kSaveLanguageDefaultKey) as? String {
            if languageSaved != "DeviceLanguage" {
                let arrayExt = languageSaved.components(separatedBy: "_")
